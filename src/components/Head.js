    ...
    <div className="grid grid-flow-col shadow-lg p-5 m-2">
      <div className="flex col-span-1">
        <img
          ...
          alt="menu"
          src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRr-cwMeLLj8MfIo3HoXJKFcOFB1g9U4DDMHA&usqp=CAU"
        />
        <a href="">
          <img
            className="h-8 mx-2 "
      ...
      <div className="col-span-10 px-10">
        <div>
          <input
            type="text"
            className="w-2/3 border px-5  border-gray-400 rounded-l-full py-2"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            onFocus={() => setShowSuggestions(true)}
            onBlur={() => setShowSuggestions(false)}
          />
          <button className="border border-gray-400 bg-gray-100 rounded-r-full py-2 px-5">
            🔍
          </button>
        </div>
        {showSuggestions && (
          <div className="fixed bg-white py-2 w-[36.5rem] shadow-lg rounded-lg border border-gray-200">
            <ul>
              {suggestions.map((s) => (
